...
func (s *searchService) HybridSearch(ctx context.Context, query string, topK int, user *model.User) ([]model.SearchResponseDTO, error) {
	...
	// 获取相关性阈值配置
	minScore := config.Conf.Elasticsearch.MinRelevanceScore
	if minScore == 0 {
		minScore = 0.5 // 默认阈值
	}
	log.Infof("[SearchService] 应用相关性阈值过滤, 最小分数: %.2f", minScore)
	...
	for _, hit := range esResponse.Hits.Hits {
		...
		if hit.Score < minScore {
			filteredCount++
			log.Debugf("[SearchService] 过滤低相关性结果: score=%.4f < %.2f, chunkID=%s", hit.Score, minScore, hit.Source.ChunkID)
			continue
		}
	...
	if filteredCount > 0 {
		log.Infof("[SearchService] 过滤了 %d 条低相关性结果", filteredCount)
	}
	log.Infof("[SearchService] 组装最终响应成功, 返回 %d 条结果 (原始 %d 条)", len(results), len(esResponse.Hits.Hits))
