...
func (s *chatService) StreamResponse(
	...
	metadataMsg := map[string]interface{}{
		"type":          "metadata",
		"has_knowledge": hasKnowledge,
		"result_count":  len(results),
		"rag_mode":      ragMode,
		"message":       getMetadataMessage(hasKnowledge, ragMode),
	}
	...
	if hasKnowledge {
		...
		if ragMode == "strict" {
			log.Warnf("[ChatService] [严格模式] 未找到相关知识库内容，将提示 LLM 拒绝回答")
		} else {
			log.Infof("[ChatService] [灵活模式] 未找到相关知识库内容，允许 LLM 使用通用知识回答")
		}
	}
...
// getMetadataMessage 根据是否有知识库内容和RAG模式返回提示信息
func getMetadataMessage(hasKnowledge bool, ragMode string) string {
	if hasKnowledge {
		return "已从知识库中检索到相关内容"
	}
	if ragMode == "flexible" {
		return "知识库中未找到相关内容，AI 将使用通用知识回答"
	}
...
func (s *chatService) buildSystemMessage(contextText string, ragMode string) string {
	...
	if ragMode == "flexible" {
		...
		if rules == "" {
			rules = config.Conf.LLM.Prompt.RulesFlexible
		}
	} else { // strict 模式
		rules = config.Conf.AI.Prompt.Rules
		if rules == "" {
